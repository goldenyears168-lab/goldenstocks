...
- 指標:每輪 Δ累計量 × 價 × 1000 = 該權證成交額增量,按 標的 × 多空(購+牛=多、售+熊=空)
  累進 5 分桶 → 30 分窗 + 全日 + 最近一輪。**描述性資料、尚未回測**,不當訊號。
...
def main():
    ...
    while smoke or _now().strftime("%H:%M") < END_HHMM:   # smoke 模式不受時窗限制(一輪即退)
        ...
        top = sorted(((dayamt[(s, "多")] - dayamt[(s, "空")], s) for s in wmap), reverse=True)[:3]
        print(f"{_now():%H:%M:%S} 第{sweep}輪 掃{len(syms)}檔 {time.monotonic()-t0:.0f}s · "
              f"死{len(dead)} · 全日淨多前3: " + " ".join(f"{names.get(s, s)}{a/1e4:+,.0f}萬" for a, s in top),
              flush=True)
        if smoke:
            break
    return 0
