...
def render():
    ...
    for r in rows:
        ...
        # 壓縮顯示規則(2026-09-24 重驗 127 日 13:25 壓縮×隔夜):>+1% 已彈開=隔夜歸零(黃粗體);−1~+1 無差別(無色);
        # <−1% 弱證據(淡綠);13:20 前是盤中位置,淡灰。標籤「蓄勢隔夜」定義未動(待雙口徑重驗)。
        if r["cmp1h"] is None:
            c_cmp = "<td class='dim'>—</td>"
        else:
            _cv = r["cmp1h"]
            _early = now.strftime("%H:%M") < "13:20"
            if _cv > 1.0:
                c_cmp = (f"<td class='warnv' style='font-weight:700{';opacity:.6' if _early else ''}' "
                         f"title='已彈開 >+1%:127日隔夜 +3.6/t0.8,佔比≥10%也僅 +68 → 隔夜名單降級'>{_cv:+.2f}% 已彈</td>")
            elif _cv < -1.0:
                c_cmp = f"<td class='dn' style='opacity:{'.5' if _early else '.8'}' title='壓縮 <−1%:127日隔夜 +138/t1.7(弱證據)'>{_cv:+.2f}%</td>"
            else:
                c_cmp = f"<td class='{'dim' if _early else ''}' title='−1~+1%:127日隔夜與壓縮<0無差別(+136 vs +139),不判;13:20 前灰=盤中位置'>{_cv:+.2f}%</td>"
...
<th title="壓縮 =(現價 ÷ 近12個5分桶均價 − 1)%,需≥8桶。顯示規則(127日 13:25 壓縮×隔夜重驗):>+1% 黃粗體「已彈」=隔夜歸零(+3.6/t0.8,佔比≥10%亦僅+68)→名單降級;−1~+1% 無色=與壓縮<0無差別(+136 vs +139);<−1% 淡綠(+138/t1.7 弱證據);13:20 前淡化=盤中位置非收盤變數。蓄勢隔夜標籤仍用壓縮<0(待重驗)。">壓縮<span class="sub">對1h均%</span></th>
