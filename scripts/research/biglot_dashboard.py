...
<th title="個股日內−宇宙日內(百分點):負(綠)=相對壓著(彈簧),>+1(黃)=已彈開;軟否決件:日線弱∧已彈=毒格−31bps">即時RS</th><th title="5分窗成交金額/近5日同時段中位">量能x</th>
<th title="下跌訊號計分(命中數·明細),六格皆127日/實戰驗證:噴後過熱=30分漲≥150bps(峰後均−32) · 勿追=漲×參與跳升或大戶賣(−5~−9.6,趨勢日−32) · 機構暗退=30分大戶賣≥3千萬∧散戶參與<15%(機構主動調節,流量領先價格~2h) · 散戶虛拉=5分漲>20∧散買≥5%(留不到收盤−6~−9) · 同賣=大戶賣∧散戶賣(隔夜−28/t−6) · 破昨防線=觸昨日午後低點(−125bps/73%貫穿);≥2粗體">跌訊</th>
...
_HELP_GROUPS = [
    ...
    ("價格", [
        ...
        ("盤前試撮(共用欄)", "不另立欄:08:45~09:00 無成交時,試撮價塞進『價』欄、試撮跳空%塞『對昨收』,皆帶『試』上標;09:00開盤後自動轉為成交資料。(試撮買一/賣一改到個股詳情頁的五檔看)", "開盤前看試撮預判開盤;試撮價會被大單掛撤誘導,非確定開盤價。"),
    ...
    ("訊號計分(只收已驗證格)", [
        ("跌訊", "命中數＋明細:噴後過熱(30分漲≥150,峰後均−32)·勿追(漲×參與跳升或大戶賣,−5~−9.6,趨勢日−32)·機構暗退(30分大戶賣≥3千萬∧散＜15%)·散戶虛拉(5分漲＞20∧散買≥5%)·同賣(大戶賣∧散戶賣,−28/t−6)·破昨防線。≥2粗體。",
