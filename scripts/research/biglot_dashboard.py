...
def render():
    ...
    for r in rows:
        ...
        if r.get("bigday") is not None and r.get("retday") is not None and not r["unm"] and _tot > 0:
            ...
            if _bp >= 10 and _rp <= 0:
                _lab, _cls = "A 乾淨吸籌(大戶買·散戶未跟)", "up"
            elif _bp >= 10 and _rp >= 5:
                _lab, _cls = "B 大戶帶散戶(散戶已在場·打折)", "warnv"
            elif _bp <= -10 and _rp >= 5:
                _lab, _cls = "C 大戶倒·散戶接刀(勿抱)", "dn"
            elif _bp <= -10:
                _lab, _cls = "D 同賣(大戶賣·散戶不接)", "dn"
            else:
                _lab, _cls = "佔比未過 ±10% 門檻,不判", ""
            _bold = "font-weight:700" if abs(_dp) >= 10 else ""
...
<th class="gd" title="(全日大戶淨 − 全日散戶淨)÷ 全日成交額 = 大戶佔比 − 散戶佔比(%)。搭配大戶佔比讀:A 佔比≥+10%∧散戶≤0 = 乾淨吸籌(紅) · B 佔比≥+10%∧散戶≥+5% = 大戶帶散戶(黃,打折) · C 佔比≤−10%∧散戶≥+5% = 大戶倒散戶接刀(綠,勿抱) · D 佔比≤−10% = 同賣(綠)。127日檢定:此欄控大戶佔比後無獨立方向資訊(−41/t−1.9),只當換手方向描述,排序仍用大戶佔比。|值|≥10% 粗體;滑鼠移上看判讀。">大戶−散戶<span class="sub">÷成交% · A/B/C/D</span></th>
