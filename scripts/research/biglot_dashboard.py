...
def render():
    ...
    for r in rows:
        ...
        # 壓縮顯示規則(2026-09-24 定案:與全日大戶佔比聯合判讀;127日 13:25 壓縮×佔比 隔夜 bps)
        #   大戶買(佔比≥+10)∧壓縮≤+1% → 蓄勢(多 +125~+158/t4.4);大戶買∧壓縮>+1% → 彈開(多方降級 +68/t0.3)
        #   大戶賣(佔比≤−10)∧壓縮>+0.3% → 倒後彈(空側最差 −56~−80);其餘無方向淡化。盤中收盤同規則。
        if r["cmp1h"] is None:
            c_cmp = "<td class='dim'>—</td>"
        else:
            _cv = r["cmp1h"]; _bs = r.get("bigsh_d")
            if _bs is not None and _bs >= 10 and _cv <= 1.0:
                c_cmp = (f"<td class='warnv' style='font-weight:700' title='蓄勢(多):全日大戶佔比 {_bs:+.1f}% ≥10 ∧ 壓縮 ≤+1%。"
                         f"127日隔夜 +125~+158(+0.3~+1 帶 t4.4)'>{_cv:+.2f}% 蓄勢</td>")
            elif _bs is not None and _bs >= 10:
                c_cmp = (f"<td class='warnv' style='font-weight:700;opacity:.75' title='彈開(多方降級):大戶佔比 {_bs:+.1f}% 但壓縮 >+1% 已彈開。"
                         f"127日隔夜僅 +68/t0.3(基準 +73)'>{_cv:+.2f}% 彈開</td>")
            elif _bs is not None and _bs <= -10 and _cv > 0.3:
                c_cmp = (f"<td class='warnv' style='font-weight:700' title='倒後彈(空側):全日大戶佔比 {_bs:+.1f}% ≤−10 ∧ 壓縮 >+0.3%(大戶倒完價又彈)。"
                         f"127日隔夜 −56~−80(基準 +73);非放空訊號,是勿抱'>{_cv:+.2f}% 倒後彈</td>")
            else:
                c_cmp = f"<td class='dim' title='無方向:|大戶佔比|<10% 或 大戶賣但價仍壓著(127日混合 −28~+64)'>{_cv:+.2f}%</td>"
        ...
        trs.append(
            ...
            + _sigtd + _stock_note_td(r["sid"])                        # ⑥ 訊號·筆記(最末)
            + "</tr>")
...
<th title="壓縮 =(現價 ÷ 近12個5分桶均價 − 1)%,需≥8桶;與全日大戶佔比聯合判讀(127日 13:25 壓縮×佔比,隔夜 bps,基準 +73):大戶買(佔比≥+10)∧壓縮≤+1% = 蓄勢(黃,+125~+158/t4.4) · 大戶買∧壓縮>+1% = 彈開(黃淡,多方降級 +68/t0.3) · 大戶賣(佔比≤−10)∧壓縮>+0.3% = 倒後彈(黃,空側 −56~−80,勿抱非放空) · 其餘淡化=無方向。盤中收盤同規則。">壓縮<span class="sub">×大戶佔比 · 蓄勢/彈開/倒後彈</span></th>
