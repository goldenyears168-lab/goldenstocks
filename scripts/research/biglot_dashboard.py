...
def _wrt_td(c, p, bull, bear, tip):
    ...
    return (f"<td class='{cls}' style='font-size:11px' title='{tip} · 活動量 購{c/1e4:,.0f}萬/售{p/1e4:,.0f}萬 · "
            f"簽號 多方{bull/1e4:,.0f}萬/空方{bear/1e4:,.0f}萬 · 多方占比{shr} {lab}'>"
            f"{c/1e4:,.0f}/{p/1e4:,.0f}<span class='dim' style='font-size:9px'> {shr}</span>"
...
_HELP_GROUPS = [
    ...
    ("大戶三尺度(單筆≥1000萬;全系統核心)", [
        ...
        ("權證5分 / 權證30分", "該標的底下的**活躍權證**(前一交易日成交額前 600 檔、對映到 36 檔標的;認購為主、認售稀少)近5分 / 近30分滾動窗。數字=認購/認售**成交額(活動量)**,單位萬——只代表多方/空方商品有多熱,**散戶倒認購也算認購成交**,不帶方向。小字=**簽號後多方占比**:每筆成交用當下買一/賣一判主動方(≥賣一=主動買、≤買一=主動賣、中間用tick rule),多方=主動買認購+主動賣認售、空方=主動賣認購+主動買認售,占比=多方÷(多方+空方)。**判斷字與顏色同一規則:≥60% 偏多(紅)、≤40% 偏空(綠)、40~60% 中性(灰)**。資料源:富邦 ws 逐筆(2 連線×300 檔,單線上限 300),不用 MIS、不吃 REST 配額。", "⚠純描述性、**尚未回測**,不進訊號欄。**『權證做多』看小字占比與顏色,不看購/售活動量。**機制假說:散戶主動買認購→造市商賣認購並買現股避險(機械性買盤),可能與中實桶有交集;反面它也可能只是散戶追價放大鏡——方向待影子帳驗(『大戶5分≥3千萬∧散買%<5%』對照組每窗自動記錄權證欄位與5/30分/收盤結果)。"),
