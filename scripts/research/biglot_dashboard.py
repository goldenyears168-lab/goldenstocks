...
def render():
    ...
    for r in rows:
        ...
        # 壓縮顯示規則(2026-09-24,jack 定案):有方向的訊號=黃粗體並標 多/空,無方向=淡化;盤中與收盤同一規則。
        # >+1% = 空(已彈開,127日隔夜歸零 +3.6/t0.8);<−1% = 多(壓在均價下,+138/t1.7 弱證據);−1~+1% 無差別。
        if r["cmp1h"] is None:
            c_cmp = "<td class='dim'>—</td>"
        else:
            _cv = r["cmp1h"]
            if _cv > 1.0:
                c_cmp = (f"<td class='warnv' style='font-weight:700' "
                         f"title='空:已彈開 >+1%,127日隔夜 +3.6/t0.8(佔比≥10%亦僅 +68)→ 隔夜名單降級'>{_cv:+.2f}% 空</td>")
            elif _cv < -1.0:
                c_cmp = (f"<td class='warnv' style='font-weight:700' "
                         f"title='多:壓在近1h均價下 >1%,127日隔夜 +138/t1.7(弱證據)'>{_cv:+.2f}% 多</td>")
            else:
                c_cmp = f"<td class='dim' title='−1~+1%:無方向(127日隔夜與壓縮<0無差別 +136 vs +139)'>{_cv:+.2f}%</td>"
...
<th title="壓縮 =(現價 ÷ 近12個5分桶均價 − 1)%,需≥8桶。黃粗體=有方向訊號:>+1% 標「空」(已彈開,127日隔夜歸零 +3.6/t0.8,佔比≥10%亦僅+68→名單降級);<−1% 標「多」(壓在均價下,+138/t1.7 弱證據);−1~+1% 淡化=無方向(與壓縮<0無差別)。盤中與收盤同規則。蓄勢隔夜標籤仍用壓縮<0(待重驗)。">壓縮<span class="sub">對1h均% · 多/空</span></th>
