...
def _active_tags(sid, nts):
    """即時標籤引擎目前有效(時距內且未 ✗)的標籤名集合。"""
    out = set()
    for (s_, tag), st in TAG_STATE.items():
        if s_ == sid and st["t0"] is not None and nts - st["t0"] < st["hz"] and not st["dead"]:
            out.add(tag)
    return out
...
def _score_rows(rows, mkt30, nts):
    ...
    for r in rows:
        ...
        bull_src = [(t, p) for t, p in (("主力點火", 2), ("純機構", 2), ("深接30", 1), ("深接5m", 1)) if t in act]
        if bull_src:
            best = max(bull_src, key=lambda x: x[1])
            sc += best[1]; sci.append(("大戶買[" + "·".join(t for t, _ in bull_src) + "]取最大", best[1]))
        for tag, pts in (("機構暗退", -2), ("噴後過熱", -2)):
            if tag in act:
                sc += pts; sci.append((tag, pts))
        ret_src = [t for t in ("散戶虛拉", "勿追30", "勿追5m") if t in act]
        if ret_src:
            if ("散戶虛拉" not in ret_src) and mkt30 > 0:
                sci.append(("勿追(市場30分>0,記0)", 0))
            else:
                sc -= 1; sci.append(("散戶側[" + "·".join(ret_src) + "]取一次", -1))
        if r.get("pmlow_warn"):
...
def _score_td(r):
    ...
    return (f"<td style='text-align:left;white-space:nowrap;{bg}' title='{html_mod.escape(tip, quote=True)}'>"
            ...
            f"<span class='dim'>盤</span><b class='{_c(sc)}'{big_sc}>{sc:+d}</b></td>")
